...
from chinese_ocr import extract_text_from_region

# ✅ Import the Universal Batch Translator
from chinese_translator import translate_text_batch
import shared_state
...
def clean_translation(text):
    """Removes annoying <unk> tokens and cleans up the final text."""
...
def process_chinese_manga(input_dir, output_img_dir, output_json_dir):
    try:
    ...
    print(f"📸 Processing {len(image_files)} Chinese images...")

    for idx, image_filename in enumerate(image_files, 1):
        if shared_state.stop_requested:
            print("\n🛑 Stop requested by user. Halting Chinese translation batch.")
            break
        ...
        for box, translated_text, typeset_color, original_chinese in to_typeset:
            if translated_text: 
